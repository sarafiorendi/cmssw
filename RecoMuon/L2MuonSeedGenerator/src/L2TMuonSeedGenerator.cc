...
// constructors
L2TMuonSeedGenerator::L2TMuonSeedGenerator(const edm::ParameterSet& iConfig) : 
  ...
  produces<L2TMuonTrajectorySeedCollection>(); 
}

// destructor
L2TMuonSeedGenerator::~L2TMuonSeedGenerator(){
  if (theService)   delete theService;
  if (theEstimator) delete theEstimator;
}

void L2TMuonSeedGenerator::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  const std::string metname = "Muon|RecoMuon|L2TMuonSeedGenerator";
  MuonPatternRecoDumper debug;

  auto_ptr<L2TMuonTrajectorySeedCollection> output(new L2TMuonTrajectorySeedCollection());
            ...
            if (newTSOS.isValid()){
              ...
              if(useOfflineSeed) {
                ...
                if(assoOffseed!=0) {
                  ...
                  output->push_back(L2TMuonTrajectorySeed(seedTSOS,container,alongMomentum,
...
const TrajectorySeed* L2TMuonSeedGenerator::associateOfflineSeedToL1( edm::Handle<edm::View<TrajectorySeed> > & offseeds, 
                                     std::vector<int> & offseedMap, 
                                     TrajectoryStateOnSurface & newTsos) {

  const std::string metlabel = "Muon|RecoMuon|L2TMuonSeedGenerator";
